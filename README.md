...
IBM 日本語DOS バージョンK3.3, IBM DOS バージョン J4.0, J5.0の日本語モード（PS/55モード）で動作します。DOS/V（日本語）やPC DOS（英語）でも動作することを意図して設計していますが、保証できません。

DOSプロンプトで`GETFPS5 mm-nn`（mmとnnは0から63までの数字）を実行して下さい。ディスプレイ・アダプターからmmからnnまでのバンク番号のフォントROMデータをダンプします。1バンクは128キロバイトです。例えば、512キロバイト（4メガビット）のフォントROMを全てダンプするには`GETFPS5 0-3`を実行します。
